...
def _request_with_timer(
    tap_stream_id: str, xero: XeroClient, filter_options: dict
) -> List[dict]:
...
def _make_request(
    context: Context, tap_stream_id: str, filter_options: dict = None, attempts: int = 0
) -> List[dict]:
...
class Contacts(PaginatedStream):
    def __init__(self, *args, **kwargs):
        super().__init__(
            "contacts",
            ["ContactID"],
            format_fn=transform.format_contacts,
            *args,
            **kwargs
        )
